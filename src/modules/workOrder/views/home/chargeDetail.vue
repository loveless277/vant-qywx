  ...
  <div>
    <!-- <van-sticky>
      <van-nav-bar
        ...
        left-text="返回"
        left-arrow
        @click-left="$router.push('/home/charge')"
      />
    </van-sticky> -->
    ...
    <van-field v-model="message" autosize disabled label="留言" type="textarea" placeholder="请输入留言" />
    <van-button type="info" size="small" @click="tableShow=true">查看表格</van-button>

      <h2>附件：</h2>
      <p v-for="item in fileArr" :key="item.name">
    ...
    <van-field
      label="执行人(多个)"
      :value="executor.join(',')"
      placeholder="选择执行人(重复选择多个)"
      required
      @focus="executorPicker = true"
      right-icon="cross"
      @click-right-icon="executor=[]"
    />
    <!-- 选择执行人 -->
    <van-popup v-model="executorPicker" position="bottom">
      <van-picker
        show-toolbar
        :columns="columns"
        @cancel="executorPicker = false"
        @confirm="executorConfirm"
        @change="onChange"
      />
    </van-popup>
    <van-button  type="info" @click="$router.push('/home/charge')">确定</van-button>
        <!-- 表格 -->
    <van-dialog v-model="tableShow" title="标题"   :show-cancel-button="false">
      <table
        border="1"
        cellspacing="0"
        cellpadding="0"
